-- monopoly.sql builds a database
...
CREATE TABLE PlayerStats (
	gameID integer REFERENCES Game(ID), 
	playerID integer REFERENCES Player(ID),
	playerCash integer,	-- 1,500 being the standard beginning amount
	playerLocation integer -- 1 through 40 spaces on the board
	);

CREATE TABLE HousingSituation (
	gameID integer REFERENCES Game(ID), 
	playerID integer REFERENCES Player(ID),
	propertyNumber integer, -- 1 through 28 property locations
